...
uint8_t MD5::padding[64] = {0x80};

uint32_t MD5::shift[64] = {
...
uint32_t MD5::sineDerivation[64] = {
...
void MD5::step(const std::vector<uint32_t>& inputVec) {
    ...
    for(uint32_t i = 0; i < 64; ++i) {
        ...
        buf_1 = buf_1 + rotateLeft(
            buf_0 + bit + MD5::sineDerivation[i] + inputVec[idx],
            this->shift[i]
        );
        buf_0 = temp;
    }
...
void MD5::finalize() {
    ...
    MD5::update(this->padding, padding_length);
...
std::vector<uint8_t> MD5::digestString(const std::string& input) {
    MD5 MD5;
    MD5.update(
...
std::vector<uint8_t> MD5::digestFile(const std::string& filepath) {
...
std::string MD5::toHash(const std::string& input) {
    std::vector<uint8_t> digest = MD5::digestString(input);
    std::stringstream ss;
